...
| --- | --- | --- | --- | --- |
| Train | 2000-01-03 to 2016-12-30 | 4,277 | 213,850 | Historical estimation |
| Validation | 2017-01-03 to 2020-12-31 | 1,007 | 50,350 | New parameter selection |
| Test | 2021-01-04 to 2026-02-20 | 1,289 | 64,450 | Common evaluation |
...
| --- | --- | --- | --- | --- | --- |
| Equal weight | 18.76% | 21.94% | 0.895 | 3.489% | 38.03% |
| Baseline GMV | 15.42% | 18.31% | 0.876 | 2.823% | 33.51% |
| Pipeline 1 | 16.31% | 18.44% | 0.912 | 2.807% | 32.07% |
| Pipeline 2 | 15.94% | 18.27% | 0.901 | 2.841% | 32.90% |
| Pipeline 3 | 15.94% | 18.27% | 0.901 | 2.843% | 32.96% |
...
| Strategy | CAGR | Volatility | Sharpe | CVaR95 | Max drawdown |
| --- | --- | --- | --- | --- | --- |
| Equal weight | 12.11% | 16.85% | 0.763 | 2.407% | 20.72% |
| Baseline GMV | 9.16% | 13.15% | 0.733 | 1.862% | 18.63% |
| Pipeline 1 | 9.68% | 13.04% | 0.774 | 1.845% | 18.23% |
| Pipeline 2 | 8.04% | 13.10% | 0.656 | 1.835% | 17.88% |
| Pipeline 3 | 8.04% | 13.08% | 0.656 | 1.833% | 17.78% |
...
| --- | --- | --- | --- | --- | --- |
| Equal weight | 1.102 | 3.778% | 0.83 | 50.00 | 0.507 |
| Baseline GMV | 1.043 | 2.881% | 3.23 | 12.41 | 0.512 |
| Pipeline 1 | 1.102 | 2.835% | 4.30 | 11.59 | 0.512 |
| Pipeline 2 | 0.932 | 2.881% | 3.25 | 17.12 | 0.528 |
| Pipeline 3 | 0.933 | 2.874% | 3.18 | 17.63 | 0.527 |
...
| --- | --- | --- | --- | --- |
| Historical downside | 3050 | 61 | 0.952 | 0.521 |
| CatBoost | 3050 | 61 | 0.884 | 0.580 |
...
| --- | --- | --- | --- |
| Prices and regimes | 0.874 | 0.577 | 0.572 |
| Plus fundamentals | 0.875 | 0.577 | 0.562 |
| Full information | 0.884 | 0.580 | 0.579 |
...
| --- | --- | --- | --- |
| Pipeline 1 − Baseline GMV | Volatility pp | -0.1042 | [-0.2170, -0.0042] |
| Pipeline 1 − Baseline GMV | CVaR95 pp | -0.0171 | [-0.0537, +0.0111] |
| Pipeline 1 − Baseline GMV | Drawdown pp | -0.3959 | [-2.6773, +1.0606] |
| Pipeline 1 − Baseline GMV | Sharpe | +0.0409 | [-0.0368, +0.1347] |
| Pipeline 2 − Pipeline 1 | Volatility pp | +0.0580 | [-0.0723, +0.2178] |
| Pipeline 2 − Pipeline 1 | CVaR95 pp | -0.0093 | [-0.0523, +0.0405] |
| Pipeline 2 − Pipeline 1 | Drawdown pp | -0.3470 | [-2.2947, +4.9699] |
| Pipeline 2 − Pipeline 1 | Sharpe | -0.1177 | [-0.2749, +0.0185] |
| Pipeline 3 − Pipeline 2 | Volatility pp | -0.0161 | [-0.0274, -0.0037] |
| Pipeline 3 − Pipeline 2 | CVaR95 pp | -0.0024 | [-0.0152, +0.0116] |
| Pipeline 3 − Pipeline 2 | Drawdown pp | -0.1035 | [-0.3175, +0.2638] |
| Pipeline 3 − Pipeline 2 | Sharpe | +0.0007 | [-0.0117, +0.0116] |
...
| --- | --- | --- | --- | --- |
| Equal weight | 12.20% | 12.11% | 11.97% | 11.74% |
| Baseline GMV | 9.52% | 9.16% | 8.63% | 7.76% |
| Pipeline 1 | 10.15% | 9.68% | 8.97% | 7.80% |
| Pipeline 2 | 8.39% | 8.04% | 7.51% | 6.64% |
| Pipeline 3 | 8.38% | 8.04% | 7.52% | 6.67% |
...
[1] Markowitz, H. (1952). Portfolio selection. The Journal of Finance, 7(1), 77–91. [Source](https://doi.org/10.1111/j.1540-6261.1952.tb01525.x).

[2] Ledoit, O., and Wolf, M. (2004). A well-conditioned estimator for large-dimensional covariance matrices. Journal of Multivariate Analysis, 88(2), 365–411. [Source](https://doi.org/10.1016/S0047-259X(03)00096-4).

[3] Jagannathan, R., and Ma, T. (2003). Risk reduction in large portfolios: Why imposing the wrong constraints helps. The Journal of Finance, 58(4), 1651–1683. [Source](https://doi.org/10.1111/1540-6261.00580).

[4] DeMiguel, V., Garlappi, L., and Uppal, R. (2009). Optimal versus naive diversification: How inefficient is the 1/N portfolio strategy? The Review of Financial Studies, 22(5), 1915–1953. [Source](https://doi.org/10.1093/rfs/hhm075).

[5] Hamilton, J. D. (1989). A new approach to the economic analysis of nonstationary time series and the business cycle. Econometrica, 57(2), 357–384. [Source](https://doi.org/10.2307/1912559).

[6] Ang, A., and Bekaert, G. (2002). International asset allocation with regime shifts. The Review of Financial Studies, 15(4), 1137–1187. [Source](https://doi.org/10.1093/rfs/15.4.1137).

[7] Asness, C. S., Frazzini, A., and Pedersen, L. H. (2019). Quality minus junk. Review of Accounting Studies, 24(1), 34–112. [Source](https://doi.org/10.1007/s11142-018-9470-2).

[8] Prokhorenkova, L., Gusev, G., Vorobev, A., Dorogush, A. V., and Gulin, A. (2018). CatBoost: Unbiased boosting with categorical features. Advances in Neural Information Processing Systems, 31. [Source](https://proceedings.neurips.cc/paper/2018/hash/14491b756b3a51daac41c24863285549-Abstract.html).

[9] Amihud, Y. (2002). Illiquidity and stock returns: Cross-section and time-series effects. Journal of Financial Markets, 5(1), 31–56. [Source](https://doi.org/10.1016/S1386-4181(01)00024-6).

[10] Rockafellar, R. T., and Uryasev, S. (2000). Optimization of conditional value-at-risk. Journal of Risk, 2(3), 21–41. [Source](https://doi.org/10.21314/JOR.2000.038).

[11] Arnott, R. D., Harvey, C. R., and Markowitz, H. (2019). A backtesting protocol in the era of machine learning. The Journal of Financial Data Science, 1(1), 64–74. [Source](https://doi.org/10.3905/jfds.2019.1.064).

[12] Künsch, H. R. (1989). The jackknife and the bootstrap for general stationary observations. The Annals of Statistics, 17(3), 1217–1241. [Source](https://doi.org/10.1214/aos/1176347265).

[13] Kaggle contributor jacksaleeby. S&P 500 OHCLV Stock Data. Kaggle dataset, version 1 used in the local experiment. Source page accessed September 6, 2026; source-file hash is retained in the repository. [Source](https://www.kaggle.com/datasets/jacksaleeby/s-and-p500-historical-data).

[14] US Securities and Exchange Commission. EDGAR application programming interfaces. Company Facts data format and API documentation. Accessed September 6, 2026. [Source](https://www.sec.gov/search-filings/edgar-application-programming-interfaces).

[15] APA Corporation (2021). Form 8-K12B, filed March 2, 2021. Holding-company reorganization and predecessor identity. [Source](https://www.sec.gov/Archives/edgar/data/1841666/000119312521063695/d127090d8k12b.htm).

[16] BlackRock, Inc. (2024). Form 8-K, filed October 1, 2024. Holding-company reorganization and successor reporting structure. [Source](https://www.sec.gov/Archives/edgar/data/1364742/000119312524229654/d856279d8k.htm).

[17] scikit-learn developers. LedoitWolf estimator documentation. Scaled-identity shrinkage and implementation reference. Accessed September 6, 2026. [Source](https://scikit-learn.org/stable/modules/generated/sklearn.covariance.LedoitWolf.html).

[18] CatBoost developers. Common training parameters. Chronological input handling and regression configuration. Accessed September 6, 2026. [Source](https://catboost.ai/docs/en/references/training-parameters/common).
...
| --- | --- |
| Universe and schedule | 50 fixed stocks; monthly formation; long only; fully invested; 15% cap |
...
| Quality | Four domains; nine ratios; lambda 1; missing rank 0.5 |
