...
def verify_frozen(c,spec):
 assert spec['input_hashes']==c['hashes'],'Price inputs changed after freeze'
 assert spec['code_hashes']==code_hashes(),'Shared implementation changed after freeze; repeat validation'
 assert spec['selection_split']=='2017-2020'
 assert spec['fundamental_key']==c['fundamental_key']
 if 'ohlcv_source' in c:assert spec['ohlcv_source']==c['ohlcv_source'],'OHLCV inputs changed after freeze'

def run_test(c,spec):
 verify_frozen(c,spec);results=run_controls(c,'test')
 ...
 pred,audit,imp=forecast_risk(c['panel'],dates,depth=ml['depth'],feature_set='full',iterations=ml['iterations'],model_dir=ROOT/'artifacts/models')
