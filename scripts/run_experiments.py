...
import os,sys,json,pickle,copy,tempfile
...
def run():
 ...
 if not np.allclose(p10.returns.net_return,results['pipeline1'].returns.net_return,rtol=0,atol=1e-11):raise ValueError('Zero-quality-strength control differs from Pipeline 1')
 if not np.allclose(p20.returns.net_return,results['pipeline2'].returns.net_return,rtol=0,atol=1e-11):raise ValueError('Zero-ML-strength control differs from Pipeline 2')
...
if __name__=='__main__':
 ...
 write_json(manifest_path,{**previous,'status':'experiments_running','notebook_execution':'pending'})
 try:
  run()
 except BaseException as exc:
  current=json.loads(manifest_path.read_text())
  write_json(manifest_path,{**current,'status':'failed','failure_stage':'experiments','error':f'{type(exc).__name__}: {exc}'})
  raise
